...
def guardar_infraestructura():
    try:
        ...
        sql_infraestructura = """INSERT INTO infraestructura 
                                (fecha, municipio, corregimiento, vereda, operador, institucion, sede, codigo_sede, direccion, barrio, comuna, zona, tipo_racion, focalizacion, observacion_general, resultado)
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        # Insertar datos en la tabla infraestructura
        cursor.execute(sql_infraestructura, (fecha, municipio, corregimiento, vereda, operador, institucion_id, sede_id, codigo_sede, direccion, barrio, comuna, zona, tipo_racion, focalizacion, observacion_general, resultado))
...
def exportar_infraestructura():
    ...
    try:
        ...
        consultas = {
            ...
            """,
            "Dotación de Menaje": """
                SELECT i.id_infraestructura, p.numero AS pregunta_numero, dm.item AS dotacion_item, dm.cantidad, 
                    dm.estado, dm.propiedad, dm.foto_menaje
                FROM infraestructura i
                LEFT JOIN dotacion_menaje dm ON i.id_infraestructura = dm.id_infraestructura
                LEFT JOIN preguntas_infraestructura p ON dm.item = p.descripcion
                WHERE 1=1
            """,
        ...
        # Si no hay datos, evitar descargar un archivo vacío
        if not dataframes:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            # Primera hoja: Infraestructura, Respuestas, y Firmas
            infra_respuestas_firmas = pd.merge(dataframes["Infraestructura"], dataframes["Respuestas Infraestructura"], on="id_infraestructura", how="left")
            infra_respuestas_firmas = pd.merge(infra_respuestas_firmas, dataframes["Firmas Infraestructura"], on="id_infraestructura", how="left")
            infra_respuestas_firmas.to_excel(writer, index=False, sheet_name="Infraestructura y Respuestas")

            # Segunda hoja: Infraestructura, Dotación de Menaje, y Firmas
            infra_dotacion_firmas = pd.merge(dataframes["Infraestructura"], dataframes["Dotación de Menaje"], on="id_infraestructura", how="left")
            infra_dotacion_firmas = pd.merge(infra_dotacion_firmas, dataframes["Firmas Infraestructura"], on="id_infraestructura", how="left")
            infra_dotacion_firmas.to_excel(writer, index=False, sheet_name="Dotación de Menaje y Firmas")
    ...
    try:
        ...
        sql_infraestructura = """
            SELECT i.id_infraestructura, i.fecha, i.municipio, i.corregimiento, i.vereda, o.nombre AS nombre_operador, 
                   ...
                   i.focalizacion, i.observacion_general, i.resultado
            FROM infraestructura i
            JOIN instituciones inst ON i.institucion = inst.id_institucion
            JOIN operadores o ON i.operador = o.id_operador
            JOIN sedes s ON i.sede = s.id_sede
...
# Luego, guárdalo en la base de datos
        ...
        for key in request.form:
            if key.startswith("item_"):
                ...
                # Procesar archivo de foto si se subió uno
                file = request.files.get(f"foto_{menaje_id}")
                if file and file.filename != '' and allowed_file(file.filename):
                    ...
                    # Obtener nombre anterior de la foto (si hay)
                    cursor.execute("""
                        SELECT foto_menaje FROM dotacion_menaje 
                        WHERE id = %s AND id_infraestructura = %s
                    """, (menaje_id, id_infraestructura))
                    result = cursor.fetchone()

                    if result and result.get("foto_menaje"):
                        old_filepath = os.path.join(upload_folder, result["foto_menaje"])
                    ...
                    new_filename = f"INF_{id_infraestructura}_menaje_{menaje_id}{ext}"
                    file.save(os.path.join(upload_folder, new_filename))

                    # Actualizar nombre de archivo en la BD
                    cursor.execute("""
                        UPDATE dotacion_menaje 
                        SET foto_menaje = %s 
                        WHERE id = %s AND id_infraestructura = %s
                    """, (new_filename, menaje_id, id_infraestructura))



        upload_folder = os.path.join(current_app.root_path, 'static/uploads/infraestructura/')
        if not os.path.exists(upload_folder):
            os.makedirs(upload_folder)
        ...
        for key in request.form:
            if key.startswith("respuesta_"):
                ...
                # Procesar archivo adjunto (foto o PDF)
                file = request.files.get(f"foto_{respuesta_id}")
                if file and file.filename != '' and allowed_file(file.filename):
                    # Obtener nombre anterior de la foto (si hay)
                    cursor.execute("""
                        SELECT foto FROM respuestas_infraestructura 
                        WHERE id_respuesta = %s AND id_infraestructura = %s
                    """, (respuesta_id, id_infraestructura))
                    result = cursor.fetchone()
                    if result and result.get("foto"):
                        old_filepath = os.path.join(upload_folder, result["foto"])
                        if os.path.exists(old_filepath):
                            os.remove(old_filepath)
                    ...
                    filename = secure_filename(file.filename)
                    ext = os.path.splitext(filename)[1].lower()
                    new_filename = f"INF_{id_infraestructura}_resp_{respuesta_id}{ext}"
                    file.save(os.path.join(upload_folder, new_filename))
                    ...
                    cursor.execute("""
                        ...
                        SET foto = %s 
                        WHERE id_respuesta = %s AND id_infraestructura = %s
                    """, (new_filename, respuesta_id, id_infraestructura))
