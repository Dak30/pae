from flask import Blueprint, render_template, request, jsonify, redirect, url_for, send_file, flash, current_app
...
from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer, Frame, KeepInFrame, Image
...
def detalle_tecnica(id_visita_tecnica):
    ...
    if request.method == 'POST':
        ...
        if accion == "guardar_pdf":           
            ...
            # Crear un contenedor para la foto de la alcaldía a la izquierda
 ...
 # Añadir el código de barras y texto
...
def editar_visita(id_visita_tecnica):
    print(f"ID Visita Técnica recibido: {id_visita_tecnica}") 
    ...
    if request.method == 'POST':
        ...
        # 📌 Obtener `UPLOAD_FOLDER` de `current_app.config` DENTRO de la función
        
        UPLOAD_FOLDER = os.path.join(current_app.root_path, 'static/uploads/tecnica')
        os.makedirs(UPLOAD_FOLDER, exist_ok=True)  # Asegurar que la carpeta existe

        # Configurar UPLOAD_FOLDER en Flask (si aún no está definido)
        ...
        for archivo in archivos:
            if archivo and allowed_file(archivo.filename):
                ...
                try:
                    ...
                    sql_archivo = """
                        INSERT INTO archivos_visita_tecnica (visita_tecnica_id, nombre_archivo, ruta_archivo, fecha_subida)
                        VALUES (%s, %s, %s, NOW())
                    """
                    cursor.execute(sql_archivo, (id_visita_tecnica, filename, f"{UPLOAD_FOLDER}/{filename}"))
                    print(f"✅ Archivo guardado: {filename}")
    ...
    cursor.execute("""SELECT vt.id_visita_tecnica, vt.fecha_visita, vt.hora_visita, vt.focalizacion, 
                    op.nombre, ins.id_institucion, ins.sede_educativa, sed.id_sede, sed.nombre_sede, 
                    vt.tipo_racion_tecnica, vt.observacion_general 
                    FROM visita_tecnica vt
    ...
    cursor.execute("SELECT * FROM archivos_visita_tecnica WHERE visita_tecnica_id = %s", (id_visita_tecnica,))
    archivos = cursor.fetchall()
    ...
    return render_template('editar_tecnica.html', operadores =operadores, instituciones=instituciones, tipos_racion = tipos_racion, visita=visita, preguntas_respuestas=preguntas_respuestas, 
                           toma_peso=toma_peso, temperaturas=temperaturas, componente_alimentario = componente_1, archivos = archivos,
