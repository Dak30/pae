...
from iniciasesion import iniciasesion_bp, login_required, session, role_required, registrar_auditoria, generar_nonce
...
import secrets
import logging
from base_datos.database import backup_database
...
def informe():
    ...
    query = """
        SELECT
            ...
            GROUP_CONCAT(DISTINCT instituciones_sedes.id_institucion SEPARATOR ', ') AS id_institucion,
...
def base_operador():
    ...
    intercambios_enriquecidos = []
    for intercambio in intercambios:
        archivos_pdf = intercambio[8].split(',') if intercambio[8] else []
        archivos_soporte = intercambio[9].split(',') if intercambio[9] else []
        archivos_aprobar = intercambio[10].split(',') if intercambio[10] else []
    ...
    try:
        ...
        if rol_usuario in ['administrador', 'nutricionista']:
            query = """
                ...
                GROUP BY operadores.nombre, tiporacion.descripcion, intercambios.numero_intercambio, intercambios.asunto, intercambios.mensaje
                   ...
                   # Crear la carpeta principal "soporte" si no existe
                    ...
                    for pdf in pdf_adjuntos:
                        if pdf and pdf.filename.lower().endswith('.pdf'):
                            ...
                            # Insertar directamente un nuevo registro SIN actualizar
                            cursor.execute("""
                                INSERT INTO archivos (id_intercambio, nombre_archivo, tipo_archivo, ruta_archivo)
                                VALUES (%s, %s, %s, %s)
...
@visitas.errorhandler(500)
def internal_error(error):
    return f"500 error: {error}\n{traceback.format_exc()}"

@visitas.route('/backup', methods=['GET'])
def create_backup():
    backup_file = backup_database()
    return jsonify({"message": "Respaldo creado", "file": backup_file}), 200
